[Link to question](https://leetcode.com/problems/earliest-finish-time-for-land-and-water-rides-i/description/)
Similar Question 2: [Link](https://leetcode.com/problems/earliest-finish-time-for-land-and-water-rides-ii/description/)
Question
---
...
// min time by a combination(i,j)= min(
// max(endTime(i)+duration(j),endTime(j)),   max(endTime(j)+duration(i),endTime(i))
// )

// simplify
// endtime(i)=start(i)+duration(i)
// max(endTime(i)+duration(j),endTime(j)) = max(start(i)+duration(i)+duration(j), start(j)+duration(j))
// => max(start(i)+duration(i), start(j)) + duration(j)

// other one: 
// => max(start(j)+duration(j), start(i)) + duration(i)
